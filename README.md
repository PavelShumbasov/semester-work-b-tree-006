...
- **B-tree** имеет следующие свойства (**t** — параметр дерева, называемый минимальной степенью B-tree, не меньший **2**.):
    ...
    - Каждый узел дерева, кроме листьев, содержащий ключи **k<sub>1</sub>,...,k<sub>n</sub>**, имеет **n+1** сына. **i**-й сын содержит ключи из отрезка   
      **[k<sub>i-1</sub>;k<sub>i</sub>],k<sub>0</sub>=−∞,k<sub>n+1</sub>=+∞.**
